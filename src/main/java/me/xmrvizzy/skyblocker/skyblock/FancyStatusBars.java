...
public class FancyStatusBars extends DrawableHelper {
    ...
    public boolean render(MatrixStack matrices, int scaledWidth, int scaledHeight) {
        ...
        if (!SkyblockerMod.getInstance().config.bars.enableBars() || player == null)
            return false;
        ...
        for (int i = 0; i < 4; i++) {
            int configAnchorNum = switch (i) {
                case 0 -> SkyblockerMod.getInstance().config.bars.barpositions.healthBarPosition().toInt();
                case 1 -> SkyblockerMod.getInstance().config.bars.barpositions.manaBarPosition().toInt();
                case 2 -> SkyblockerMod.getInstance().config.bars.barpositions.defenceBarPosition().toInt();
                case 3 -> SkyblockerMod.getInstance().config.bars.barpositions.experienceBarPosition().toInt();
                default -> 0;
            };
